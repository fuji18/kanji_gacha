...
**説明**: [救済](#救済-rescue)（ヒント・交換）のコストはこのガチャ残りを消費する（レベル別 0/-1/-2。交換は1枚あたり×枚数）。
...
**説明**: 上限は暫定12（要調整）。同一部品を複数枚持てるため、手札内は `HandPart`（instanceId付き）で管理する。上限到達時はUIがガチャボタンを非活性化する。ゲーム開始時は上限（または引ける残数）まで自動で補充される（初期手札・`SessionManager.fillHand`）。
...
**定義**: 詰まりを打開するための最小限の手段。MVPは**「ヒント」と「交換」の2つだけ**。

**説明**: v1.0企画の3層救済（ピティ天井・相性ボーナス・レスキューガチャ・再振りチケット等）は**廃止**。詰みは[床の統計的維持](#床の統計的維持-floor)で予防する設計に転換した。

**関連用語**: [ヒント](#ヒント-hint)、[交換](#交換-exchange)、[床の統計的維持](#床の統計的維持-floor)
...
**定義**: 選択した手札 1枚以上 を一括で引き直す打開操作（旧称：捨てて引き直す）。交換後の手札枚数は不変。

**説明**: 達成型（deck）は選択枚数を山札へ戻して切り直し同数を引き直す（部品は場から失われない）。タイムアタックは1枚分＝(1)削除＋コスト消費 →(2)補充ガチャ の2ステップを枚数ぶん適用し、総コスト（レベル別 やさ無料／ふつう-1／むず-2 ×枚数）が残不足なら実行しない（消費もしない）。補充は通常のガチャ残を減らさない。KPI フィールド名は互換のため `discardUsed` を維持。

**実装**: `SessionManager.exchangeCards` / `RescueService.discardAndDraw`
