...
- **CI への `npm audit` 追加は見送り**: `ci.yml` は「順序変更禁止」の明記があるパイプラインで、
  audit の失敗は依存更新（Dependabot PR）でしか解消できず、無関係な PR を赤くするノイズになる。
  依存監査は Dependabot（npm/actions 週次・minor+patch グループ化）で担い、
  audit の CI ゲート化は脆弱性対応の運用が回り始めてから再検討する。
