...
- **URL レバーと記録の正当性（T-060）**：E2E 用レバーのうちゲームバランスを変えるもの
  （`?taMs`/`?deckMax`）の使用時は、セッションを**記録対象外**（ベスト・図鑑・にがてを永続化しない）
  とする（`SessionManagerOptions.recordable`）。`?seed` は乱数の再現のみでバランス不変のため記録対象のまま。
  **Phase2 でランキング等のバックエンドを導入する際は、URL レバーの全無効化とサーバー側のスコア検証を
  必須とする**（connect-src 開放と同時に設計に含めること）
