...
import type { CombineEntry, KanjiEntry, Part, Rarity } from './types';
import {
  LEVELS,
  LEVEL_RANK,
  MAX_COMBINE_PARTS,
...
/** kanji.json のアセット圧縮：読み・意味の上限（約100KB以内・PRD性能要件） */
const MAX_READINGS = 6;
const MAX_MEANINGS = 2;

/** 部品文字を昇順（コードポイント順）に並べる比較関数 */
...
function buildCombineDict(
  ...
  for (const [char, comps] of Object.entries(decomp)) {
    // 合体不能（2部品未満）・探索上限超（MAX_COMBINE_PARTS 超）は辞書から除外する
  ...
  return [...byKey.entries()]
    .map(([key, set]): CombineEntry => {
      ...
      const cands = results.map((c) => kanjiByChar.get(c)!);
      return {
        key,
        results,
        primary: selectPrimary(cands).char,
...
function buildParts(
  ...
  // 各部品について：2部品エントリ数（weight 素）と作れる漢字の最小level を集計する
  interface Agg {
  ...
  // weight 分布からレアリティの分位境界を決める（暫定・T-009 で調整）
  ...
  // 汎用度が高い（weight 大）ほど rarity 1。境界は weight の分位点。
  const r1 = q(0.2);
  const r2 = q(0.4);
  const r3 = q(0.6);
  const r4 = q(0.8);
...
function writeJson(name: string, data: unknown): number {
  const path = join(PUBLIC_DATA_DIR, name);
