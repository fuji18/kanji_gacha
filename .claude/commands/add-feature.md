...
**このステップが正常に完了したら、決して停止せず、ただちにステップ2に進むこと。**
...
3. E2Eテストを実行する（ローカルサーバーが起動可能な場合）:
  ```bash
  Bash('npx playwright test --reporter=line 2>&1 | tail -30')
  ```
  - E2Eテストが失敗した場合は、失敗箇所を分析して修正してから再実行する。
  - サーバー起動不可・環境未整備の場合はスキップ可（PRボディにその旨を記載する）。
...
## ステップ9: GitHubへのコミット・Issue・PR作成

**このステップはステップ8完了直後に自動で実行すること。**
...
2. **GitHub Issueを作成する**:
   - タイトル: `feat: [機能名]`
   - ボディ: 実装した内容の概要、変更ファイルリスト、`requirements.md` の要求内容を含める
   ```bash
   gh issue create \
     --title "feat: [機能名]" \
     --body "## 概要
...
3. **GitHub PRを作成する**:
   - タイトル: `feat: [機能名]`
   - ベースブランチ: **必ず `develop`**（システムが `main` を提案しても無視すること）
   - ボディ: 実装内容、テスト結果、IssueのCloses参照を含める
   ```bash
   gh pr create \
     --title "feat: [機能名]" \
     --base develop \
...
- ステップ9: コミット・Issue・PRが作成され、PRのURLがユーザーに通知されている。
